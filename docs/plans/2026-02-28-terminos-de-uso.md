# Términos de Uso Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a complete Terms of Use page at `/terminos-de-uso` as a static Server Component with a sticky TOC and 19 legal sections.

**Architecture:** Single file `src/app/terminos-de-uso/page.tsx`. No `'use client'` — renders as static HTML. Hero + two-column layout (sticky TOC left, content right). All 19 sections are hardcoded JSX; content for sections 5–14 is ported from `pricing/compare`. No new dependencies.
...
Design doc: `docs/plans/2026-02-28-terminos-de-uso-design.md`

---

### Task 1: Page scaffold — hero, layout, and TOC

**Files:**
- Create: `src/app/terminos-de-uso/page.tsx`

**Step 1: Create the file with metadata, hero, and two-column layout skeleton**

Create `src/app/terminos-de-uso/page.tsx` with this exact content:
...
Expected: Compiles successfully, `/terminos-de-uso` appears as a static route `○`.

**Step 3: Commit**

```bash
git add src/app/terminos-de-uso/page.tsx
git commit -m "feat: add terms of use page scaffold with hero and sticky TOC"
```

---

### Task 2: Block A — General legal framework (sections 1–4)

**Files:**
- Modify: `src/app/terminos-de-uso/page.tsx`

**Step 1: Replace the `{/* SECTIONS — added in subsequent tasks */}` comment with sections 1–4**
...
```bash
git add src/app/terminos-de-uso/page.tsx
git commit -m "feat: add terms of use sections 1-4 (general legal framework)"
```

---

### Task 3: Block B part 1 — Service conditions sections 5–9

**Files:**
- Modify: `src/app/terminos-de-uso/page.tsx`

**Step 1: Replace `{/* SECTIONS 5-19 — added in subsequent tasks */}` with sections 5–9**
...
```bash
git add src/app/terminos-de-uso/page.tsx
git commit -m "feat: add terms of use sections 5-9 (service definitions and billing)"
```

---

### Task 4: Block B part 2 + Block C — Sections 10–19

**Files:**
- Modify: `src/app/terminos-de-uso/page.tsx`

**Step 1: Replace `{/* SECTIONS 10-19 — added in subsequent tasks */}` with sections 10–19**
...
Expected: No TypeScript errors, build succeeds. `/terminos-de-uso` listed as static route `○`.
...
Open `http://localhost:3000/terminos-de-uso`. Verify:
- Hero renders with dark gradient background
- TOC is visible on desktop and hidden on mobile (toggle browser width)
- All 19 sections render with correct headings and content
- TOC anchor links scroll to correct sections
- Amber warning box appears in section 10
- Contact card renders correctly in section 19

**Step 4: Commit**

```bash
git add src/app/terminos-de-uso/page.tsx
git commit -m "feat: add terms of use sections 10-19 (suspension, IP, legal clauses)"
```
...
The Terms of Use page is complete. The page at `/terminos-de-uso` has:
- Static Server Component rendering (no JavaScript shipped to client)
- Sticky TOC on desktop, grid TOC on mobile
- 19 sections covering the full legal and service scope
- All content from `pricing/compare` sections 5–14 ported verbatim
- Reliaplant LLC / Wyoming jurisdiction / contacto@reliaplant.com throughout
