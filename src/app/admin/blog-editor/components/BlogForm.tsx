...
export default function BlogForm({
  ...
  useEffect(() => {
    const fetchContributors = async () => {
      try {
        ...
        const activeContributors = data.filter(
          (contributor) => contributor.active
        );
  ...
  const handleKeyPhraseAdd = () => {
    if (
      newKeyPhrase.trim() &&
      !blogPost.keyPhrases?.includes(newKeyPhrase.trim())
    ) {
      setBlogPost((prev) => ({
        ...prev,
  ...
  const handleKeyPhraseRemove = (phraseToRemove: string) => {
    setBlogPost((prev) => ({
      ...prev,
      keyPhrases:
        prev.keyPhrases?.filter((phrase) => phrase !== phraseToRemove) || [],
    }));
  };
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" &&
                  (e.preventDefault(), handleKeyPhraseAdd())
                }
              />
              <button
                type="button"
