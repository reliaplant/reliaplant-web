  ...
  return (
    <div className="container mx-auto p-4 w-[60vw] mt-[10vh] pb-[10vh]">
      ...
      <div className="border-b-2 border-black pb-2 mb-24 flex flex-row items-center">
        <div>
          <span>
            <span className="p-1 px-2 bg-blue-200 text-blue-600">Artículo técnico</span> - Autor: RP Tech Team
          </span>
        </div>
        <div>
          <Image
      ...
      <div className="prose max-w-none">
        <ReactMarkdown
          remarkPlugins={[remarkGfm]} // Add this to enable GFM (tables, etc.)
            components={{
            h1: ({ node, ...props }) => <h1 className="text-4xl font-bold my-4 text-gray-900" {...props} />,
            h2: ({ node, ...props }) => <h2 className="text-3xl font-bold my-3 text-gray-800 mt-12 uppercase mb-8 mt-16" {...props} />,
            h3: ({ node, ...props }) => <h3 className="text-2xl uppercase font-bold my-2 text-gray-800 mt-12" {...props} />,
            p: ({ node, ...props }) => <p className="my-4 text-lg text-gray-600 leading-relaxed" {...props} />,
            ...
            th: ({ node, ...props }) => <th className="px-4 py-2 text-lg text-left font-semibold bg-gray-100 border" {...props} />,
            td: ({ node, ...props }) => <td className="px-4 py-2 text-lg border" {...props} />,
          }}
        >
          {post.content}
        </ReactMarkdown>
